...
internal class RoundedBarChart : BarChart {
    ...
    private fun readRadiusAttr(context: Context, attrs: AttributeSet) {
        val a = context.theme.obtainStyledAttributes(attrs, be.yarin.goalify.R.styleable.RoundedBarChart, 0, 0)
        try {
            setRadius(a.getDimensionPixelSize(be.yarin.goalify.R.styleable.RoundedBarChart_radius, 0))
        } finally {
            a.recycle()
        }
    }
